...
export async function GET(
  ...
  const ZERION_API_URL = `https://api.zerion.io/v1/wallets/${address}/pnl`;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: 'Failed to fetch P&L data from Zerion', details: errorData }, { status: response.status });
    }
